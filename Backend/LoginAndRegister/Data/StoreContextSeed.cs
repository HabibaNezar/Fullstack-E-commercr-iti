...
public static class StoreContextSeed
{
    public static async Task SeedAsync(AppDbContext context, UserManager<AppUser> userManager)
    {
        try
        {
            // 1. إضافة الأقسام أولاً (لأنها لا تعتمد على وجود مستخدم)
            ...
            // 2. البحث عن بائع أو مستخدم لإضافته كـ SellerId
            var allSellers = await userManager.GetUsersInRoleAsync("Seller");
            var seller = allSellers.FirstOrDefault();

            if (seller == null)
            {
                // لو ملقاش Seller، نحاول نجيب الـ Admin اللي لسه مكرتينه في Program.cs
                seller = await context.Users.FirstOrDefaultAsync();
            }

            // لو لسه مفيش يوزر خالص في السيستم (وده مستبعد لو الـ Admin اتكريت صح)
            // هنوقف إضافة المنتجات بس نكون ضمنا إن الـ Categories اتضافت
